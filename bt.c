...
static int bt_test_new(bt_test_t ** test,
    bt_suite_t * suite,
    const char * dlname,
    size_t prefix_len);
  ...
  while (elf_cur) {
    ...
    while (suite_cur) {
          ...
          for (int i = 0, flag = 0; i < BT_PASS_MAX; i++) {
            if (test_cur->results[i] > BT_TEST_NONE) {
              ...
              switch (i) {
                ...
                case BT_PASS_TEST:
                  dprintf(self->fd, "%stest%s ", self->color ? CYAN : "", self->color ? ENDCOL : "");
                  break;
              ...
              switch (test_cur->results[i]) {
                ...
                case BT_TEST_IGNORED:
                  dprintf(self->fd,
                    ...
                    self->color ? ENDCOL : ""); break;
                case BT_TEST_FAILED:
                  dprintf(self->fd, "%sfailed%s", self->color ? RED : "", self->color ? ENDCOL : "");
                  break;
      ...
      if (count)
        dprintf(
            self->fd,
            "  => %d tests, %d succeeded (%g%%), %d ignored (%g%%), %d failed (%g%%), %d corrupted (%g%%)\n",
            count,
            results[BT_TEST_SUCCEEDED],
